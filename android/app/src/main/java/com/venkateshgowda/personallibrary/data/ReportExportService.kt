...
object ReportExportService {
    ...
    fun workbook(books: List<BookEntity>, wishlist: List<WishlistEntity>, loans: List<LoanEntity> = emptyList(), today: String = "9999-12-31"): ByteArray = ByteArrayOutputStream().use { bytes ->
        ZipOutputStream(bytes).use { zip ->
            ...
            entry(zip, "xl/workbook.xml", """<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Summary" sheetId="1" r:id="rId1"/><sheet name="Books" sheetId="2" r:id="rId2"/><sheet name="Wishlist" sheetId="3" r:id="rId3"/><sheet name="Loans" sheetId="4" r:id="rId4"/></sheets></workbook>""")
            entry(zip, "xl/_rels/workbook.xml.rels", """<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet2.xml"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet3.xml"/><Relationship Id="rId4" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet4.xml"/></Relationships>""")
            entry(zip, "xl/worksheets/sheet1.xml", worksheet(listOf(listOf("Library Report Summary"), listOf("Total Books", books.size.toString()), listOf("Total Investment INR", "%.2f".format(books.sumOf { it.pricePaise } / 100.0)), listOf("Average Price INR", "%.2f".format(if (books.isEmpty()) 0.0 else books.sumOf { it.pricePaise } / 100.0 / books.size)), listOf("Favourite Books", books.count { it.favourite }.toString()), listOf("Active Loans", loans.count { it.actualReturnDate == null }.toString()), listOf("Overdue Loans", loans.count { it.actualReturnDate == null && it.expectedReturnDate != null && it.expectedReturnDate < today }.toString()), listOf("Wishlist Items", wishlist.size.toString()))))
            entry(zip, "xl/worksheets/sheet2.xml", worksheet(buildList<List<String>> { add(listOf("Title", "Author", "Category", "Price INR", "Purchase Date", "Publisher", "ISBN", "Language", "Rating", "Status", "Favourite")); addAll(books.map { listOf(it.title, it.author, it.category.orEmpty(), "%.2f".format(it.pricePaise / 100.0), it.purchaseDate.orEmpty(), it.publisher.orEmpty(), it.isbn.orEmpty(), it.language.orEmpty(), it.rating?.toString().orEmpty(), it.readingStatus, it.favourite.toString()) }) }))
            entry(zip, "xl/worksheets/sheet3.xml", worksheet(buildList<List<String>> { add(listOf("Title", "Author", "Category", "Expected Price INR", "Priority", "Expected Purchase Date", "Status", "Notes")); addAll(wishlist.map { listOf(it.title, it.author.orEmpty(), it.category.orEmpty(), "%.2f".format(it.expectedPricePaise / 100.0), it.priority, it.expectedPurchaseDate.orEmpty(), it.status, it.notes.orEmpty()) }) }))
            entry(zip, "xl/worksheets/sheet4.xml", worksheet(buildList<List<String>> { add(listOf("Book", "Author", "Borrower", "Contact", "Borrowed Date", "Expected Return Date", "Actual Return Date", "Status", "Notes")); addAll(loans.map { loan -> val book = books.firstOrNull { it.id == loan.bookId }; listOf(book?.title ?: "Deleted book", book?.author.orEmpty(), loan.borrowerName, loan.borrowerContact.orEmpty(), loan.borrowedDate, loan.expectedReturnDate.orEmpty(), loan.actualReturnDate.orEmpty(), loanStatus(loan, today), loan.notes.orEmpty()) }) }))
        }
        bytes.toByteArray()
    }
